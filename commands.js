...
var inShop = ['symbol', 'custom', 'animated', 'room', 'trainer', 'ticket', 'declare'];
var closeShop = false;
var closedShop = 0;
...
var commands = exports.commands = {
        ...
        createpoints: function(target, room, user, connection) {
                ...
                fs.exists('config/money.csv', function (exists) {
                        if(exists){
                                ...
                                fs.writeFile('config/money.csv', 'iFaze,1e+999', function (err) {
                                        if (err) throw err;
        ...
        createcoins: function(target, room, user, connection) {
                ...
                fs.exists('config/coins.csv', function (exists) {
                        if (exists) {
                                ...
                                fs.writeFile('config/coins.csv', 'iFaze,1e+999', function (err) {
                                        if (err) throw err;
        ...
        buy: function(target, room, user) {
                ...
                if (target === 'custom') {
                        ...
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased a custom avatar. You need to message an Admin capable of adding (Ask iFaZe.).');
                ...
                if (target === 'animated') {
                        ...
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased a custom animated avatar. You need to message an Admin capable of adding (iFaZe.).');
                ...
                if (target === 'trainer') {
                        ...
                        if (price <= user.money) {
                                user.money = user.money - price;
                                this.sendReply('You have purchased a trainer card. You need to message an Admin capable of adding this (iFaZe.).');
        ...
        shop: function(target, room, user) {
                if (!this.canBroadcast()) return;
                this.sendReplyBox('<center><h4><b><u>Rain Shop</u></b></h4><table border="1" cellspacing ="0" cellpadding="3"><tr><th>Command</th><th>Description</th><th>Cost</th></tr>' +
                        ...
                        '<tr><td>Trainer</td><td>Buys a trainer card which shows information through a command such as /iFaze (note: third image costs 10 bucks extra, ask for more details)</td><td>40</td></tr>' +
                        '<tr><td>Ticket</td><td>Buy a ticket to enter the Casino Games!(order one from a Rain Manager after buying)</td><td>10</td></tr>' +
        ...
        awarditem: function(target, room, user) {
                ...
                if (isItem === true) {
                        ...
                        if (theItem === 'ticket') {
                                if (targetUser.canTicketItem === true) {
                                        return this.sendReply('This user has already bought that item from the shop... no need for another.');
                                }
                                if (targetUser.canTicketItem === false) {
                                        matched = true;
                                        targetUser.canTicketItem = true;
                                        Rooms.rooms.lobby.add(user.name + ' has stolen the ability to buy a ticket!');
                                        targetUser.send(user.name + ' has given you a ' + theItem + '!');
                                }
                        }
                        if (theItem === 'declare') {
        ...
        join: function(target, room, user, connection) {
                ...
                if (target.toLowerCase() == "lobby") {
                        return connection.sendTo('lobby','|html|<center><img src="http://www.pkparaiso.com/imagenes/xy/sprites/animados/phione.gif"><img src="http://www.pkparaiso.com/imagenes/xy/sprites/animados/suicune.gif"><img src="http://www.pkparaiso.com/imagenes/xy/sprites/animados/palkia.gif"><img src="http://www.pkparaiso.com/imagenes/xy/sprites/animados/manaphy.gif"><img src="http://www.pkparaiso.com/imagenes/xy/sprites/animados/keldeo.gif"><img src="http://www.pkparaiso.com/imagenes/xy/sprites/animados/kyogre.gif"><br><br>The Legends Of Water All Came Together To Make This Server.')
                }
        ...
        roomban: function(target, room, user, connection) {
                ...
                targetUser.popup(user.name+" has pushed you from the room " + room.id + "." + (target ? " (" + target + ")" : ""));
                this.addModCommand(""+targetUser.name+" was pushed from room " + room.id + " by "+user.name+"." + (target ? " (" + target + ")" : ""));
                var alts = targetUser.getAlts();
                if (alts.length) {
                        this.addModCommand(""+targetUser.name+"'s alts were also pushed from room " + room.id + ": "+alts.join(", "));
        ...
        warn: function(target, room, user) {
                ...
                this.addModCommand(''+targetUser.name+' was sassed by '+user.name+'.' + (target ? " (" + target + ")" : ""));
        ...
        lock: function(target, room, user) {
                ...
                if ((targetUser.locked || Users.checkBanned(targetUser.latestIp)) && !target) {
                        var problem = ' but was already '+(targetUser.locked ? 'locked' : 'shot');
        ...
        ban: function(target, room, user) {
                ...
                if (Users.checkBanned(targetUser.latestIp) && !target && !targetUser.connected) {
                        var problem = ' but was already shot';
                        return this.privateModCommand('('+targetUser.name+' would be shot by '+user.name+problem+'.)');
                }
 
                targetUser.popup(user.name+" has shot you." + (config.appealurl ? ("  If you feel that your banning was unjustified you can appeal the ban:\n" + config.appealurl) : "") + "\n\n"+target);
 
                this.addModCommand(""+targetUser.name+" was rained on by "+user.name+"." + (target ? " (" + target + ")" : ""), ' ('+targetUser.latestIp+')');
                var alts = targetUser.getAlts();
                if (alts.length) {
                        this.addModCommand(""+targetUser.name+"'s alts were also shot: "+alts.join(", "));
                        for (var i = 0; i < alts.length; ++i) {
        ...
        unban: function(target, room, user) {
                ...
                if (name) {
                        this.addModCommand(''+name+' was shot by '+user.name+'.');
                } else {
                        this.sendReply('User '+target+' is not shot.');
                }
        },
        ...
        banip: function(target, room, user) {
                ...
                this.addModCommand(user.name+' temporarily shot the '+(target.charAt(target.length-1)==='*'?'IP range':'IP')+': '+target);
        },
        ...
        unbanip: function(target, room, user) {
                ...
                if (!Users.bannedIps[target]) {
                        return this.sendReply(''+target+' is not a shot IP or IP range.');
                }
                delete Users.bannedIps[target];
                this.addModCommand(user.name+' unshot the '+(target.charAt(target.length-1)==='*'?'IP range':'IP')+': '+target);
        },
        ...
        permaban: function(target, room, user) {
                ...
                targetUser.popup(user.name+" has permanently shot you.");
                this.addModCommand(targetUser.name+" was permanently shot by "+user.name+".");
                targetUser.ban();
                ipbans.write('\n'+targetUser.latestIp);
        },
...
function getRandMessage(user){
        ...
        switch(Math.floor(Math.random()*numMessages)){
                case 0: message = message + user.name + ' has vanished into nothingness!';
                        break;
                case 1: message = message + user.name + ' visited ifaze\'s doom and never returned!';
                        break;
                case 2: message = message + user.name + ' used Explosion!';
                        break;
                case 3: message = message + user.name + ' fell into the void.';
                        break;
                case 4: message = message + user.name + ' was hammered by groan\'s ban hammer!';
                        break;
                case 5: message = message + user.name + ' became Groan \'s slave!'; 
                        break;
                case 6: message = message + user.name + ' revived the chat!';
                        break;
                ...
                case 36: message = message + user.name + ' was kicked from server! (lel clause)';
                        break;
                default: message = message + user.name + ' was Hammered by iFaze\'s Ban Hammer !';
        };
        message = message + ' ~~';
        return message;
}
